...
(function () {
  ...
  function clearGalleryMasonry(gallery) {
    if (!gallery) return;

    gallery.classList.remove("is-masonry-ready");
  ...
  function layoutGalleryMasonry(gallery, token) {
    if (!gallery || token !== galleryLayoutToken || !gallery.isConnected) return;

    const items = Array.from(gallery.querySelectorAll(".gallery-item"));
    const columnCount = getGalleryColumnCount();

    clearGalleryMasonry(gallery);

    // One column is more stable as normal document flow on narrow screens.
    if (columnCount === 1 || !items.length) return;
    ...
    // Widths must be committed before measuring intrinsic media heights.
    items.forEach((item) => {
  ...
  function scheduleGalleryMasonry(gallery, token) {
    if (!gallery) return;
    ...
    galleryResizeFrame = requestAnimationFrame(() => {
      ...
      layoutGalleryMasonry(gallery, token);
    });
  }

  function destroyGalleryLayout() {
    galleryLayoutToken += 1;
  ...
  function renderGallery(items, type) {
    const list = items.map((item, index) => {
      const aspectClass = item.aspect === "portrait" ? "gallery-item--portrait" : item.aspect === "square" ? "gallery-item--square" : item.aspect === "wide" ? "gallery-item--wide" : item.aspect === "tall" ? "gallery-item--tall" : "gallery-item--landscape";
      ...
      if (type === "photography") {
        ...
        return `
          <button type="button" class="gallery-item ${aspectClass}" data-index="${index}" data-type="photography" aria-label="Open ${item.title}">
            ${imageMarkup}
      ...
      return `
        <button type="button" class="gallery-item ${aspectClass}" data-index="${index}" data-type="videography" aria-label="Open ${item.title}">
          ${videoMarkup}
          <span class="gallery-item__overlay"><span class="gallery-item__view">VIEW</span></span>
  ...
  function renderCategory(category) {
    ...
    const renderToken = galleryLayoutToken;
    projectPanel.classList.add("is-transitioning");
    setTimeout(() => {
      if (renderToken !== galleryLayoutToken) return;
  ...
  function attachGalleryEvents() {
    ...
    const gallery = projectPanel ? projectPanel.querySelector(".gallery") : null;
    const layoutToken = galleryLayoutToken;
    ...
    galleryItems.forEach((item) => {
      ...
      function updateAspectRatio(width, height) {
        if (!width || !height || layoutToken !== galleryLayoutToken) return;

        const ratio = width / height;
        item.style.aspectRatio = `${width} / ${height}`;
        item.classList.remove("gallery-item--portrait", "gallery-item--landscape", "gallery-item--square", "gallery-item--wide", "gallery-item--tall");

        if (ratio > 1.5) item.classList.add("gallery-item--wide");
        else if (ratio > 1) item.classList.add("gallery-item--landscape");
        else if (Math.abs(ratio - 1) < 0.1) item.classList.add("gallery-item--square");
        else if (ratio > 0.6) item.classList.add("gallery-item--portrait");
        else item.classList.add("gallery-item--tall");

        scheduleGalleryMasonry(gallery, layoutToken);
      }
